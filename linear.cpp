// SSMU.cpp (BRAM-SLASHED: fuse huge FIFOs into one stage, write DDR in-place, minimal depths)
// Key changes:
// 1) Remove ddA_stream / dB_stream / H1_stream / HC_to_final (these were HUGE BRAM FIFOs)
// 2) Fuse: (A_to_ddA + B_to_dB + update_H + join + final_output) into ONE stage:
//    - Reads X_gate[VEC_D], dX[VEC_D], delta[VEC_D]
//    - For each i: read A_i, B_i, C_i
//    - For each j: read H0(i,j), compute H1(i,j), write H1_ddr/C_ddr, accumulate acc[j] += H1*C
//    - After all i: out[j] = X_gate[j] + acc[j]
// 3) Depths are minimized (just enough to avoid handshake jitter; no waste).
// Notes:
// - Keeps DDR transaction counts: still writes HUGE_LEN entries to both C_ddr and H1_ddr
// - Output order preserved: out is VEC_D tokens (j-minor order)
// - No “JT multiple” constraint anymore.
// - This is the most BRAM-efficient dataflow form for your computation.
...
#pragma HLS PIPELINE II=1
            ...
            // Compute H1 = H0*exp(A*delta) + (B*delta)*dX
            DTYPE_VEC H1v;
            for (int l = 0; l < VEC_FACTOR; ++l) {
#pragma HLS UNROLL
                float a  = (float)vget(A_vec, l);
                float dl = (float)vget(dlt,  l);
                float ddA = hls::expf(a * dl);

                ACC_T H0  = (ACC_T)vget(H0v, l);
                ACC_T Bx  = (ACC_T)vget(B_vec, l);
                ACC_T dX  = (ACC_T)vget(dx,   l);

                ACC_T H1  = H0 * (ACC_T)ddA + (Bx * (ACC_T)dl) * dX;
                vset(H1v, l, (DTYPE)H1);
            }
...
#pragma HLS STREAM variable=delta_stream       depth=256   // VEC_D tokens (e.g. 160) -> 256 safe
